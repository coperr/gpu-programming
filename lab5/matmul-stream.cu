...
int main()
{
  ...
  for (int j = 0; j < N; j++) {
    ...
    cudaMemcpyAsync(&dB[streamId * N], &B[j * N], N * sizeof(float), cudaMemcpyHostToDevice, streams[streamId]);
    ...
    matvec<<<gridSize, blockSize, 0, streams[streamId]>>>(dA, &dB[streamId * N], &dC[streamId * N], N);

    // Copy back the computed vector dC(:, j % nStreams) into the column C(:, j) using the same stream no (j % nStreams) and cudaMemcpyAsync
    cudaMemcpyAsync(&C[j * N], &dC[streamId * N], N * sizeof(float), cudaMemcpyDeviceToHost, streams[streamId]);
  }
