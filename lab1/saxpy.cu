...
int main(int argc, char **argv)
{
  ...
  printf("Running saxpyBlocsThreads with blockSize %d...\n", blockSize);
  ...
  blockSize = 256;
  k = 4;
  int totalThreadsNeeded = (N + k - 1) / k; 
  int numBlocksK = (totalThreadsNeeded + blockSize - 1) / blockSize;

  printf("Running saxpyBlocsThreadsKops with blockSize %d and k=%d...\n", blockSize, k);
  saxpyBlocsThreadsKops<<<numBlocksK, blockSize>>>(N, a, dx, dy, k);
