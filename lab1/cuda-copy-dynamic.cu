...
int main(int argc, char **argv) {
  ...
  // TODO / A FAIRE ...
  cudaError_t err = cudaMalloc((void **) &dA, N * sizeof(float));

  // cudaMemcpy from A[N] to dA[N]
  // cudaMemcpy de A[N] vers dA[N]
  // TODO / A FAIRE ...
  err = cudaMemcpy(dA, A, N*sizeof(float), cudaMemcpyHostToDevice);

  // cudaMemcpy from dA[N} to B[N]
  // cudaMemcpy de dA[N] vers B[N]
  // TODO / A FAIRE ...
  err = cudaMemcpy(B, dA, N*sizeof(float), cudaMemcpyDeviceToHost);
