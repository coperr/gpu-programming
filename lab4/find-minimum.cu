...
__global__ void findMinimum(float *dA, float *dAmin, int N)
{
  ...
  for (int s = BLOCKSIZE / 2; s > 0; s >>= 1) {
    if (tid < s) {
      buff[tid] = fminf(buff[tid], buff[tid + s]);
    }
    __syncthreads();
  }
...
int main()
{
  ...
  findMinimum<<<numBlocks2, BLOCKSIZE>>>(dAmin, dA, numBlocks);
  cudaMemcpy(Amin, dA, numBlocks2 * sizeof(float), cudaMemcpyDeviceToHost);
