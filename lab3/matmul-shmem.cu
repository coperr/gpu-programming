...
__global__ void multiplyMatrixGPUByBlocksThreads2DNonMultipleSharedMemory(float *dA, float *dB, float *dC, int n)
{
  ...
  for (int i = 0; i < (n + BSXY - 1) / BSXY; ++i) {

    int RowA = bx * BSXY + ty;
    int ColA = i * BSXY + tx;
    if (RowA < n && ColA < n) {
       shA[ty][tx] = dA[RowA * n + ColA];
    } else {
       shA[ty][tx] = 0.0f;
    }
...
int main(int argc, char **argv)
{
  ...
  auto run_kernel = [&](const char* name, auto kernel_call) {
      ...
      std::cout << name << " Time: " << milliseconds << " ms, Performance: " << gflops << " GFlops/s" << std::endl;
  };
...

  {
    ...
    run_kernel("GPU By Blocks (1 thread)", [&]() { 
  ...
  }
  {
    ...
    run_kernel("GPU By Blocks Threads 1D", [&]() {
  ...
  { 
    ...
    run_kernel("GPU By Blocks Threads 1D Non Multiple", [&]() {
  ...
  }
  {
    ...
    run_kernel("GPU By Blocks Threads 2D", [&]() {
  ...
  }
  {
    ...
    run_kernel("GPU By Blocks Threads 2D Non Multiple", [&]() {
...

  {
    ...
    run_kernel("GPU Shared Memory Tiled", [&]() {
